<div class="flex-col">
    <div class="relative {{ $errors->has($name) ? 'mb-2' : 'mb-6' }}" data-te-input-wrapper-init>
        <input type="{{ $type }}" id="{{ $name }}"
            class="
           @error($name) border-red-400 @enderror
           peer w-full rounded border
           btn-font h-[44px] inline-flex items-center
           text-secondary
           bg-transparent px-3 py-[0.32rem] leading-[1.6] outline-none transition-all duration-200 ease-linear
        ...
        <label for={{ $name }}
            ...
            truncate pt-[0.37rem] leading-[1.6] text-secondary
            transition-all duration-200 ease-out peer-focus:-translate-y-[0.9rem] peer-focus:scale-[0.8] 
            peer-focus:text-accent peer-data-[te-input-state-active]:-translate-y-[0.9rem]
